...
```{r,message=FALSE}
EMS_table_final <- left_join(EMS_table_1, EMS_table_2); EMS_table_final
```
...
```{r}
NYPD_data %>% filter(Final_Incident_Type == "1. Critical") %>% group_by(year(Week_Start_Date))  %>% summarize(num_of_Incidents_Calculated = sum(num_of_Incidents_Calculated))
```
...
```{r}
NYPD_data %>% filter(Final_Incident_Type == "2. Serious") %>% group_by(year(Week_Start_Date))  %>% summarize(num_of_Incidents_Calculated = sum(num_of_Incidents_Calculated))
```
...
```{r}
NYPD_data %>% filter(Final_Incident_Type == "3. Non-Critical") %>% group_by(year(Week_Start_Date))  %>% summarize(num_of_Incidents_Calculated = sum(num_of_Incidents_Calculated))
```
...
```{r, results='hide'}
forecast(ts_EMS_final_1_diff, h = 52)
```

```{r}
autoplot(forecast(ts_EMS_final_1_diff, h = 52))
```
...
```{r}
FDNY_data %>% filter(Final_Incident_Type == "1. Structural Fires") %>% group_by(year(Week_Start_Date))  %>% summarize(num_of_Incidents_Calculated = sum(num_of_Incidents_Calculated))
```
...
```{r}
FDNY_data %>% filter(Final_Incident_Type == "2. Non-Structural Fires") %>% group_by(year(Week_Start_Date))  %>% summarize(num_of_Incidents_Calculated = sum(num_of_Incidents_Calculated))
```
