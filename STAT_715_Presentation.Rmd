...
autoplot(forecast(ts_EMS_final_1_fit_arima, h = 52)) + labs(title = "ARIMA(3,1,1) Forecast of Life Threatening Emergencies Data", y = "Seconds")
```
...
```{r, fig.width=10, fig.height=7}
autoplot(forecast(ts_NYPD_data_1_fit_arima, h = 52)) + labs(title = "MA(3) with Drift Forecast of Critical Inicidents Data",y = "Seconds")
```

# FDNY

```{r, echo=FALSE}
FDNY_table_final %>% kable(caption = "Number of Incidents Occured", align = 'c') %>% kable_classic(full_width = F) %>% kable_styling(bootstrap_options = "striped")
```
...
Let us plot the differenced Structural Fires data:
...
Our Q-statistic is again, less than $\chi^2$ at 95% confidence with 50 degrees of freedom. We fail to reject the null hypothesis and conclude $H_0$
...
autoplot(forecast(ts_FDNY_data_fit), h = 52) + labs(title = "MA(1) with Drift Forecast of Structural Fires Data", y = "Seconds")
```

# Thank you. Any questions? I would appreciate any constructive criticism.
